...
# История сообщений (системный промт)
chat_history = [
    {
        "role": "system",
...
def main(page: ft.Page):
                ...
                ft.Row(
                    [
                        ...
                        ft.IconButton(
                            ...
                            icon_size=16,
                            tooltip="Переответить",
                            on_click=lambda e: regenerate_last_response(),
                        ),
                    ],
    ...
    add_message("AI Помощник", "Привет! Чем могу помочь сегодня?", is_user=False)

    def send_prompt(prompt_prefix="", is_regen=False):
        if not is_regen:
        ...
        try:
            ...
            # Запрос к Apinex через библиотеку openai
            ...
            add_message("Ошибка", f"`{str(err)}`", is_user=False)
            ...
            [
                ft.Row(
                    [
                        ...
                        ft.Text(
                            "AI Assistant", size=18, weight=ft.FontWeight.BOLD
                        ),
                    ]
                ),
                ft.IconButton(
        ...
        [
            ft.Chip(
                ...
                on_click=lambda e: send_prompt(
                    "Разбери этот код на Python и найди ошибки:"
                ),
            ),
            ft.Chip(
                label=ft.Text("💡 Тема"),
                on_click=lambda e: send_prompt("Объясни просто и наглядно тему:"),
            ),
            ft.Chip(
                label=ft.Text("📝 Тест"),
                on_click=lambda e: send_prompt("Сделай мини-тест из 3 вопросов по:"),
            ),
        ],
